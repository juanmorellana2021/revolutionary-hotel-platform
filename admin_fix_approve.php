...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'], $_POST['experience_id'])) {
    ...
    if ($action === 'approve') {
        $stmt = $pdo->prepare("UPDATE aini_experiences SET status = 'approved', reviewed_at = NOW(), reviewed_by = ? WHERE id = ?");
        ...
        $stmt = $pdo->prepare("UPDATE aini_experiences SET status = 'rejected', rejection_reason = ?, reviewed_at = NOW(), reviewed_by = ? WHERE id = ?");
        $stmt->execute([$reason, $_SESSION['user_id'], $experienceId]);
    ...
    <div class="container">
        <div class="header">
            ...
            <div class="stats">
                ...
                <div class="stat-card">
                    <h3><?php 
                        $activeCount = $pdo->query("SELECT COUNT(*) FROM aini_experiences WHERE status = 'approved'")->fetchColumn();
                        echo $activeCount;
                    ?></h3>
                    <p>Active Experiences</p>
                </div>
            </div>
            <a href="public_booking.php" class="btn-back" style="margin-top: 15px;">← Back to Home</a>
        </div>
                        ...
                        <div>
                            ...
                            <div class="info-section" style="margin-top: 20px;">
                                ...
                                <div class="info-row">
                                    ...
                                    <span><?php echo number_format($exp['price_aini_rewards'] ?? 0); ?> coins</span>
                                </div>
                                <div class="info-row">
                                    <strong>AINI Crypto:</strong>
                                    <span><?php echo number_format($exp['price_aini_crypto'] ?? 0, 4); ?> AINI</span>
                                </div>
                                <?php if (($exp['discount_percentage'] ?? 0) > 0): ?>
                                <div class="info-row">
